...
def train(model, X_train, X_test, y_train, y_test, total_epoch, batch_size, learning_rate, state=None, source_dir=None, prefix=None, val_fold_number=0, writer=None):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(len(X_train))
    print(len(X_test))
    ...
    for epoch in trange(0, total_epoch):
        ...
        if(best_val * 0.995 < val_macro_f1score):
            ...
            torch.save(model.state_dict(), os.path.join(model_dir, '_'.join(['fold', str(val_fold_number-1)]) + '_' + prefix + '_epoch' + str(epoch) + 'para'))
        else:
        ...
        writer.add_scalars("hamming_loss train/test", {"hammingloss_train"+str(val_fold_number-1):hammingloss, "hammingloss_test"+str(val_fold_number-1):val_hamming}, epoch)
        writer.add_scalars("macro_f1score train/test", {"macro_f1score_train"+str(val_fold_number-1):macro_f1score, "macro_f1score_test"+str(val_fold_number-1):val_macro_f1score}, epoch)
        writer.add_scalars("micro_f1score train/test", {"micro_f1score_train"+str(val_fold_number-1):micro_f1score, "micro_f1score_test"+str(val_fold_number-1):val_micro_f1score}, epoch)
        ...
        np.save(os.path.join(log_dir, prefix + '_epoch' + str(epoch) + '_fold' + str(val_fold_number-1) + '.npy'), d)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--total_epoch', type=int, default=80, help='Total number of training epochs.')
    parser.add_argument('--batch_size', type=int, default=64, help='Batch size for training.')
    parser.add_argument('--learning_rate', type=float, default=0.000355, help='Learning rate for training.')
    ...
    val_macro_f1score = 0
    train(model, train_X, val_X, train_labels, val_labels, total_epoch=args.total_epoch, batch_size=args.batch_size, learning_rate=args.learning_rate, state=None, source_dir=source_dir, prefix=prefix, val_fold_number=val_fold_number, writer=writer)
